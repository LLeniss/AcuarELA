// i18n.js — versión "definitiva" con textos embebidos
(function () {
  // Traducciones embebidas (fallback definitivo)
  const TRANSLATIONS = {
    "_comentario": "TEXTOS DE LA PÁGINA EN 3 IDIOMAS · Edita aquí cualquier texto visible. Cada clave existe en es / en / fr. La traducción NO es automática: si cambias el español, traduce también en/fr (o deja el español si aún no tienes la traducción).",
    "es": {
      "meta_title": "Eduvina Parada Cáceres · Artista de acuarela | Su primera obra",
      "meta_desc": "Eduvina Parada Cáceres es artista. Vive con ELA bulbar. Mañana exhibe su primera acuarela al público. Adquiérela, dona o comparte y ayuda a que este camino continúe.",
      ...
      "donar_banco_l2": "N.º <strong>[000-0000-00]</strong>",
    ...
    "en": {
      ...
      "donar_banco_l2": "No. <strong>[000-0000-00]</strong>",
    ...
    "fr": {
      ...
      "donar_banco_l2": "N° <strong>[000-0000-00]</strong>",
  ...
  // Aplicador de traducciones: data-i18n, data-i18n-html, data-i18n-attr
  function applyLang(lang) {
    ...
    document.querySelectorAll("[data-i18n]").forEach((el) => {
      ...
      if (typeof value !== "undefined") {
        ...
        // si falta la clave, no tocamos el contenido existente, pero avisamos en consola
        console.warn(`[i18n] Clave faltante "${key}" para idioma "${lang}"`);
      }
    });
    ...
    document.querySelectorAll("[data-i18n-html]").forEach((el) => {
      ...
      if (typeof value !== "undefined") {
        ...
        console.warn(`[i18n] (html) Clave faltante "${key}" para idioma "${lang}"`);
      }
    });

    // data-i18n-attr -> lista de pares attr:key;attr2:key2
    document.querySelectorAll("[data-i18n-attr]").forEach((el) => {
      const spec = el.getAttribute("data-i18n-attr");
      if (!spec) return;
      spec.split(";").forEach((pairRaw) => {
        const pair = pairRaw.trim();
        if (!pair) return;
        const idx = pair.indexOf(":");
        if (idx === -1) {
          console.warn(`[i18n] Formato inválido en data-i18n-attr: "${pair}"`);
          return;
        }
        ...
        if (typeof val !== "undefined") {
          ...
          console.warn(`[i18n] (attr) Clave faltante "${key}" para idioma "${lang}"`);
        }
      });
    });

    // Opcional: actualizar metas si existen (meta_title / meta_desc)
    const metaTitle = texts["meta_title"];
    const metaDesc = texts["meta_desc"];
    if (metaTitle) {
  ...
  // Exponer setLanguage globalmente para botones/selectores
  window.setLanguage = function (lang) {
    try {
      localStorage.setItem("lang", lang);
    } catch (e) {
      // localStorage puede fallar en modos de privacidad estrictos; no bloquear
    }
    applyLang(lang);
  };

  // helper: detecta mejor lenguaje (localStorage > html lang > navigator)
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    applyLang(lang);
    // consola informativa
    console.info(`[i18n] Aplicado idioma: ${lang} (usando traducciones embebidas)`);
  });
})();
